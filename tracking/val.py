...
from typing import Dict, List # Dict 외 List
...
def parse_opt() -> argparse.Namespace:
    # --- 1. 메인 파서 생성 ---
    # 프로그램 전체 설명 등을 포함할 수 있음
    parser = argparse.ArgumentParser(description="BoxMOT Validation Pipeline")

    # --- 2. 공통 인자용 부모 파서 생성 ---
    # add_help=False: 각 subparser가 자신의 도움말을 갖도록 함
    common_parser = argparse.ArgumentParser(add_help=False)

    # --- !!! 여기에 모든 공통 인자 정의를 옮김 !!! ---
    common_parser.add_argument('--source', type=str, help='file/dir/URL/glob, 0 for webcam') # 예: 테스트셋 경로
    common_parser.add_argument('--project', default=ROOT / 'runs', type=Path, help='save results to project/name')
    common_parser.add_argument('--name', default='', help='save results to project/name')
    common_parser.add_argument('--exist-ok', action='store_true', default=True, help='existing project/name ok, do not increment')
    common_parser.add_argument('--conf', type=float, default=0.01, help='min confidence threshold')
    common_parser.add_argument('--iou', type=float, default=0.7, help='intersection over union (IoU) threshold for NMS')
    common_parser.add_argument('--device', default='', help='cuda device, i.e. 0 or 0,1,2,3 or cpu')
    common_parser.add_argument('--half', action='store_true', help='use FP16 half-precision inference')
    common_parser.add_argument('--vid-stride', type=int, default=1, help='video frame-rate stride')
    common_parser.add_argument('--agnostic-nms', default=False, action='store_true', help='class-agnostic NMS')
    common_parser.add_argument('--fps', type=int, default=None, help='video frame-rate')
    common_parser.add_argument('--gsi', action='store_true', help='apply Gaussian smooth interpolation postprocessing')
    common_parser.add_argument('--verbose', action='store_true', help='print results') # verbose도 공통으로 사용될 수 있음
    common_parser.add_argument('--ci', action='store_true', help='Automatically reuse existing due to no UI in CI')
    common_parser.add_argument('--val-tools-path', type=Path, default=EXAMPLES / 'val_utils', help='path to store trackeval repo in')
    common_parser.add_argument('--split-dataset', action='store_true', help='Use the second half of the dataset')
    # --- 공통 인자 정의 끝 ---

    subparsers = parser.add_subparsers(dest='command', help='Available commands', required=True) # required=True 추가 권장

    # --- 4. 각 Subparser 정의 (parents 사용) ---

    # Subparser for generate_dets_embs
    generate_dets_embs_parser = subparsers.add_parser(
        'generate_dets_embs',
        help='Generate detections and embeddings',
        parents=[common_parser] # *** 공통 인자 상속 ***
    )
    # --- 이 Subparser에만 필요한 특화된 인자 추가 ---
    # (주의: 공통 인자와 겹치는 것은 여기에 다시 정의하지 않음!)
    generate_dets_embs_parser.add_argument('--yolo-model', nargs='+', type=Path, default=[WEIGHTS / 'yolov8n.pt'], help='yolo model path(s)') # default 값 등은 여기서 재정의 가능
    generate_dets_embs_parser.add_argument('--reid-model', nargs='+', type=Path, default=[WEIGHTS / 'osnet_x0_25_msmt17.pt'], help='reid model path(s)')
    generate_dets_embs_parser.add_argument('--imgsz', '--img', '--img-size', nargs='+', type=int, default=None, help='inference size h,w') # default None으로 유지
    ...
    generate_mot_results_parser = subparsers.add_parser(
        'generate_mot_results',
        help='Generate MOT results',
        parents=[common_parser]
    )

    # --- 이 Subparser에만 필요한 특화된 인자 추가 ---
    generate_mot_results_parser.add_argument('--yolo-model', nargs='+', type=Path, default=[WEIGHTS / 'yolov8n.pt'], help='yolo model path(s) used for naming output') # 이름 짓기 위해 필요
    generate_mot_results_parser.add_argument('--reid-model', nargs='+', type=Path, default=[WEIGHTS / 'osnet_x0_25_msmt17.pt'], help='reid model path(s) used for naming output and tracker init')
    generate_mot_results_parser.add_argument('--tracking-method', type=str, default='deepocsort', help='tracker name')
    # generate_mot_results_parser.add_argument('--imgsz', ...) # imgsz가 이 단계에서도 필요하다면 추가
    ...
    # --- 이 Subparser에만 필요한 특화된 인자 추가 ---
    trackeval_parser.add_argument('--exp-folder-path', type=Path, required=True, help='path to experiment folder containing MOT results')
    # trackeval_parser.add_argument('--val-tools-path', ...) # 만약 여기서 기본값을 다르게 하려면 재정의 가능
    trackeval_parser.add_argument('--metrics', type=str, nargs='+', default=["HOTA", "CLEAR", "Identity"], help='Metrics to evaluate') # trackeval 전용 인자 추가 예시


    # --- 5. 인자 파싱 ---
    opt = parser.parse_args()

    # --- 6. 파싱 후 공통 로직 (source 경로 처리 등) ---
    # opt.source는 이제 어떤 subparser가 실행되든 항상 존재함 (common_parser에서 상속받았으므로)
    if opt.source: # source가 None이 아닌지 확인 (선택적 인자로 바꿀 경우 대비)
         source_path = Path(opt.source)
         # benchmark와 split 설정 (이 로직이 모든 명령어에 필요한지 확인 필요)
         if source_path.exists() and source_path.is_dir() and source_path.parent.name: # 경로 유효성 검사 강화
             opt.benchmark = source_path.parent.name # 예: 'polyp_test_mot'
             opt.split = source_path.name         # 예: 'test'
         else:
             # source가 파일이거나 다른 구조일 경우 기본값 또는 에러 처리
             log.warning(f"Could not determine benchmark/split from source path: {opt.source}")
             opt.benchmark = 'unknown'
             opt.split = 'unknown'
    else:
         # source가 필수인데 주어지지 않은 경우 argparse가 에러 처리 (required=True 가정)
         # 만약 source가 필수가 아니라면 여기서 기본값 설정
         opt.benchmark = 'unknown'
         opt.split = 'unknown'
